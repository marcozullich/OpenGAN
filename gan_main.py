...
from gan import architectures, features, datasets
from dupes import datasets as punches_data

def load_args():
    parser = argparse.ArgumentParser()
    parser.add_argument("-e", "--epochs", type=int, default=300, help="Number of epochs for the GAN training (default: 300).")
    parser.add_argument("-b", "--batch_size_train", type=int, default=128, help="Batch size for training (default: 128).") 
    parser.add_argument("-z", "--latent_dim", type=int, default=100, help="Dimension of the latent space for the generator (default: 100).")
    parser.add_argument("-w", "--base_width", type=int, default=64, help="Base width (i.e., minimum number of output channels) per hidden conv layer in discriminator and generator (default: 64).")
    parser.add_argument("-d", "--input_channel_dim", type=int, default=512, help="Number of channels of the input data (the features representation of the images --- default: 512).")
    parser.add_argument("-l", "--lr", type=float, default=0.0001, help="The base learning rate for the generator (default: 0.0001)")
    ...
    parser.add_argument("-n", "--model_name", type=str, default="GAN", help="Model name (used for saving purposes --- default: 'GAN').")
    parser.add_argument("-s", "--save_folder_params", type=str, default="models", help="Folder where the parameters will be saved. The name of the file will be `<model_name>_id.pt`, where id is a progressive int starting from 0 (default: 'models').")
...
def main():
    ...
    trainloader = DataLoader(datasets.BasicDataset(train_features))
    ...
    G_losses = [] # -> relocate to training fct
    D_losses = []
